	...
	<el-main class="bg table_wrap exam">
		...
		<el-table border :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" stripe>
			...
			<el-table-column fixed="right" label="操作" width="200">
				<template slot-scope="scope">
					<div class="view_a">
					<router-link class="e-button el-button--small is-plain el-button--primary" v-if="user_group == '管理员' || $check_action('/exam/table','set')"
						:to="'./view?' + field + '=' + scope.row[field]" size="small">
            			<span>详情</span>
					</router-link>
					<router-link class="e-button el-button--small is-plain el-button--primary" v-if="user_group == '管理员' || $check_exam('/exam/table','setting')"
								 :to="'/question_table/table?' + field + '=' + scope.row[field]" size="small">
						<span>题库</span>
					</router-link>
					<!--&& !!-->
					<router-link class="e-button el-button--small is-plain el-button--primary"  :data-num='answered_list[scope.row[field]]' style="margin-left: 0;margin-top: 10px" v-if="(user_group == '管理员' || $check_exam('/exam/table','answers')) && answered_list[scope.row[field]]"
								 :to="'/answer_view/view?' + field + '=' + scope.row[field]" size="small">
						<span>查看分数</span>
					</router-link>
					<router-link class="e-button el-button--small is-plain el-button--primary" style="margin-left: 0;margin-top: 10px" v-else-if="(user_group == '管理员' || $check_exam('/exam/table','answers')) && scope.row.status==='启用'"
								 :to="'/answer_view/view?' + field + '=' + scope.row[field]" size="small">
						<span>答题</span>
					</router-link>
					<router-link class="e-button el-button--small is-plain el-button--primary" style="margin-top: 10px" v-if="user_group == '管理员' || $check_exam('/exam/table','give_score')"
