	...
	<el-main class="bg table_wrap subject_score">
		...
		<el-table border :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" stripe>
			...
			<el-table-column fixed="right" label="操作" width="200">
				<template slot-scope="scope">
					<div class="view_a">
					<router-link v-if="scope.row['score_state']===1" class="e-button el-button--small is-plain el-button--primary"
						:to="'/exam_subject/score_view?user_answer_id=' + scope.row['user_answer_id']" size="small">
						<span>查看</span>
					</router-link>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/subject_user_answer/get_list?like=0",
				url_del: "~/api/subject_user_answer/del?",
