<template>
	<el-main class="bg table_wrap forum_type">
			...
			<el-row class="rows row2">
				<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap">
					<el-col :xs="24" :sm="12" :lg="12" class="search_btn_1 btns">
							...
							<el-button v-if="user_group == '管理员' || $check_action('/forum_type/table', 'del')"
									class="float-right search_btn_del" type="danger" @click="delInfo()">删除</el-button>
							<!-- <router-link v-if="user_group == '管理员' || $check_action('/forum_type/view')"
								class="el-button float-right el-button--default el-button--primary search_btn_add" to="./view?"><span>添加</span>
							</router-link> -->
							<el-button  v-if="user_group == '管理员' || $check_action('/forum_type/view')" class="float-right  search_btn_add" @click="$router.push('./view?')">添加</el-button>
		...
		<el-table border :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%"
			...
			<!--<el-table-column prop="img" label="图标" min-width="110">-->
			...
			<el-table-column fixed prop="name" label="类型名称" sortable min-width="120">
			</el-table-column>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/forum_type/get_list?like=0",
				url_del: "~/api/forum_type/del?",
