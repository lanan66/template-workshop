<template>
  <div class="page_exam subject_details" id="exam_view">
    <div class="warp subject_warp" v-if="$check_exam('/subject_exam/list', 'answers')">
    ...
    async post_user_answer_wrong() {
      for (let i = 0; i < this.question_list.length; i++) {
        ...
        if (exam_question.exam_question_id) {
          ...
          if (json.result) {
            this.answer_wrong_form.subject_name = json.result.obj.subject_name;
            this.answer_wrong_form.title = json.result.obj.title;
            this.answer_wrong_form.nickname = this.answer_form.nickname;
            this.answer_wrong_form.type = json.result.obj.type;
            this.answer_wrong_form.user_id = this.$store.state.user.user_id;
            this.answer_wrong_form.answer =
              json.result.obj.answer.length > 0 ? json.result.obj.answer : "";
            let obj = this.get_answer_wrong_by_id(
              this.answer_form.answers,
              exam_question.exam_question_id
            );
            this.answer_wrong_form.answers = JSON.stringify(obj.answer);
            this.answer_wrong_form.question_item =
              json.result.obj.question_item;
          }

          this.$post(
            this.url_add_user_answer_wrong,
            this.answer_wrong_form,
