<template>
	<view class="diy_details diy_div_classification_information">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','categorize_multiple')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>分类下多:</span>
						</view>
							<view class="diy_field diy_text">
							<text>
								{{obj["categorize_multiple"]}}
							</text>
						</view>
						</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get(getApp().globalData.host + "/api/classification_information/get_obj", {
					classification_information_id:this.query.classification_information_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["classification_information_id"] !==0) {
...
<style scoped>
	.div_classification_information_details {}
