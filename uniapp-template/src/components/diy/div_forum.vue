<template>
  <view class="div_forum">
    ...
    <view class="tag_block">
      标签：<text class="tag" v-for="o in obj[vm.tag]" :key="o">{{ o }}</text>
    </view> 
    <view class="doc">
  ...
  methods: {
    ...
    add_praise() {
      ...
      if (!this.check_praised) {
        ...
        this.$post(getApp().globalData.host + "/api/praise/add?", body, (res) => {
          _this.obj.praise_len += 1;
          console.log(res);
          this.$post(
            getApp().globalData.host + "/api/forum/set?forum_id=" + _this.obj.forum_id,
            {
    ...
    get_collect() {
      ...
      this.$get(getApp().globalData.host + "/api/collect/count?", query, (res) => {
        if (res.result || res.result === 0) {
    ...
    add_collect() {
      ...
      if (!this.check_collected) {
        ...
        this.$post(getApp().globalData.host + "/api/collect/add?", body, (res) => {
          this.$toast("收藏成功");
          console.log(res);
        });
      } else {
        this.check_collected = false;
        this.$get(getApp().globalData.host + "/api/collect/del", body, (res) => {
          this.$toast("取消收藏");
        });
      }
    },
