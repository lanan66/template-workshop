<template>
	<view class="page_forum" id="forum_index">
		...
		<uni-search-bar placeholder="搜索帖子" @confirm="search_" @cancel="cancel" cancelText="取消"
		...
		<view class="change">
			...
			<navigator class="button" v-if="user_group == '管理员' || $check_action('/forum/list', 'add')"
		...
		methods: {
			...
			get_forum_type() {
				this.$get("~/api/forum_type/get_list", {}, (res) => {
					if (res.result) {
						res.result.list.map((o) => this.list_forum_type.push({
							value: o.name,
							text: o.name
						}));
					}
