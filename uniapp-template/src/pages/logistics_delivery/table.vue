<template>
  <view class="page_logistics_delivery diy_table diy_table_tml" id="logistics_delivery_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <view class="table_padding_bottom">
                <button class="btn_right" size="mini" @click="reset()" style="margin-left: 10px;margin-right: 15px">重置
                </button>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/logistics_delivery/view?"
                         v-if="user_group == '管理员' || user_group == '卖家'">添加
              </navigator>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/logistics_delivery/table','del') || $check_action('/logistics_delivery/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
                  <view class="uni-padding-wrap uni-common-mt">
                    <navigator :url="'/pages/logistics_delivery/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/logistics_delivery/view','set') || $check_action('/logistics_delivery/view','get')"
                               class="btn btn-primary">详情
                    </navigator>
                    <button @click="changeSigning('logistics_delivery_id=' + o['logistics_delivery_id'], o['signing_status'], i)"
                             v-if="user_group == '管理员' || (o['signing_status'] == '待签收' && $check_option('/logistics_delivery/table', 'sign_for'))"
                             class="btn btn-primary">签收
                    </button>
                  </view>
                </view>
              </view>
...
export default {
  ...
  methods: {
    ...
    async get_list_delivery_status() {
      ['待取货', '配送中', '已完成'].map((o) => this.list_delivery_status.push({value: o, text: o}));
    },
    /**
     * 获取签收状态列表
     */
