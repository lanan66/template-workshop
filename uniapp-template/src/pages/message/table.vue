	...
	<view class="diy_table diy_table_tml" id="message_table">
		<view :hidden="userFeedbackHidden" class="popup_content">
			...
			<view class="popup_textarea_item">
				<input type="text" class="popup_textarea" value="" v-model="feedbackContent" placeholder="请输入回复内容"
					id="reply_input" />
...
<!--						<uni-th align="center">留言邮箱</uni-th>-->
					...
					<uni-tr v-for="(o, i) in list" :key="i">
						<uni-td align="center">
							<image style="width:3rem;height: 3rem;" :src="$fullImgUrl(o.avatar)" mode="scaleToFill" />
						</uni-td>
...
<!--						<uni-td align="center">{{ o.email }}</uni-td>-->
						...
						<uni-td align="center">
							<navigator :url="'/pages/message/view?message_id=' + o.message_id" class="answer_link">
								<button type="default" size="mini">详情</button>
							</navigator>

              <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/message/table','del')">删除</button>

              <button v-if="user_group === '管理员' && (o.reply == '' || o.reply ==null)" type="default" size="mini"
                      @click="showDiv($event,o.message_id,'')" class="answer_link">回复</button>
							<button v-if="o.reply != ''&& o.reply!= null" type="default" size="mini"
								@click="showDiv($event,o.message_id,o.reply)" class="answer_link">查看回复</button>
	...
	export default {
        ...
        url_del: "~/api/message/del",
				...
				query: {
					...
					page: 1,
					title: "",
					user_id: this.$store.state.user.user_group == '管理员' ? '' : this.$store.state.user.user_id
				},
      ...
      delInfo(v) {
        ...
        uni.showModal({
          ...
          success: function (res) {
			...
			showDiv(e, message_id, reply) { // 显示输入弹出框
