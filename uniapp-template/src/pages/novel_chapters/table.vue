<template>
	<view class="page_novel_chapters diy_table diy_table_tml" id="novel_chapters_table">
		<!-- 筛选模块(开始) -->
		<view>
					...
					<view>
						<view class="">
							...
							<uni-forms :modelValue="query">
								<uni-forms-item label="章节名称" name="novel_title">
									<uni-easyinput type="text" v-model="query.chapter_name" placeholder="章节名称" />
								</uni-forms-item>
							</uni-forms>
							<!-- /搜索栏 -->
							<view class="table_padding_bottom">
								<button class="btn_right" size="mini" @click="reset()"
									style="margin-left: 10px;margin-right: 15px">重置</button>
				...
				<view>
					<view>
						<view class="table_padding_top">
							<navigator class="nav_left" :url="'/pages/novel_chapters/view?novel_reading_id=' + query.novel_reading_id">
								添加</navigator>
							<div class="clear"></div>
						</view>
						<!-- 列表 -->
									...
									<view class="col">
										<view class="view">
											<view class="create_time_block diy_filed diy_time">
												<button class="delete_a_btn" style="margin: 0;" @click="delInfo(i)">删除</button>
											</view>
										</view>
									</view>
									<view class="uni-padding-wrap uni-common-mt">
										<navigator :url="'/pages/novel_chapters/view?novel_chapters_id=' + o['novel_chapters_id']"
											class="btn btn-primary">详情</navigator>
									</view>
								</view>
							</view>
						</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/novel_chapters/get_list?like=0",
