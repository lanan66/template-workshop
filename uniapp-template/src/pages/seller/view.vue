          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','store_name') || ($check_field('add','store_name') || $check_field('set','store_name'))" label="店铺名称" name="store_name">
                            <uni-easyinput type="text" v-model="form['store_name']" v-if="user_group === '管理员' || (form['seller_id'] && $check_field('set','store_name')) || (!form['seller_id'] && $check_field('add','store_name'))" :disabled="disabledObj['store_name_isDisabled']" />
                <!-- 仅查看 -->
            ...
            <view class="form_button" v-if="$check_action('/seller/view','set') || ($check_action('/seller/view','add') || $check_option('/seller/table','examine'))">
...
import mixin from "@/libs/mixins/page.js";
  ...
  data() {
    return {
      ...
      is_password: true,
      field: "seller_id",
      url_add: "~/api/seller/add?",
      url_set: "~/api/seller/set?",
      url_get_obj: "~/api/seller/get_obj?",
  ...
  methods: {
	...
	,uploadFile_(type) {
		...
		,successChoose(filePath, type) {
			uni.uploadFile({
				url: getApp().globalData.host + '/api/seller/upload', //仅为示例，非真实的接口地址
				filePath,
				name: 'file',
    ...
    submit_sub(form_sub){
      if(form_sub["seller_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?seller_id=" + form_sub["seller_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/seller/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/seller/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/seller/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/seller/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/seller/view','get');
